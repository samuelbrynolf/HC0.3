...
(function(c,f,g){c.tapHandling=false;var d=function(h){return h.each(function(){var s=f(this),m,p,q,r,i=10;function k(t){f(t.target).trigger("tap",[t,f(t.target).attr("href")]);t.stopImmediatePropagation()}function o(v){var t=v.originalEvent||v,u=t.touches||t.targetTouches;if(u){return[u[0].pageX,u[0].pageY]}else{return null}}function j(u){if(u.touches&&u.touches.length>1||u.targetTouches&&u.targetTouches.length>1){return false}var t=o(u);q=t[0];p=t[1]}function l(u){if(!r){var t=o(u);if(t&&(Math.abs(p-t[1])>i||Math.abs(q-t[0])>i)){r=true}}}function n(t){clearTimeout(m);m=setTimeout(function(){c.tapHandling=false;r=false},1000);if((t.which&&t.which>1)||t.shiftKey||t.altKey||t.metaKey||t.ctrlKey){return}t.preventDefault();if(r||c.tapHandling&&c.tapHandling!==t.type){r=false;return}c.tapHandling=t.type;k(t)}s.bind("touchstart.tappy MSPointerDown.tappy",j).bind("touchmove.tappy MSPointerMove.tappy",l).bind("touchend.tappy MSPointerUp.tappy",n).bind("click.tappy",n)})};var a=function(h){return h.unbind(".tappy")};if(f.event&&f.event.special){f.event.special.tap={add:function(h){d(f(this))},remove:function(h){a(f(this))}}}else{var b=f.fn.bind,e=f.fn.unbind;f.fn.bind=function(h){if(/(^| )tap( |$)/.test(h)){d(this)}return b.apply(this,arguments)};f.fn.unbind=function(h){if(/(^| )tap( |$)/.test(h)){a(this)}return e.apply(this,arguments)}}}(this,jQuery));(function(){$("html").addClass("transitions");function h(){if($("#getActiveMQ-watcher").length){var n=$(window);var m=0;var o=$("#js-mq-example");var l=o.text();function k(){o.text("Yey! Alpha mode!")}function j(){var p=getActiveMQ();$("#js-mq-currentexample").text("Current screen: "+p);if(p=="aq"){k()}else{o.text(l)}if(!(p=="aq")&&!(p=="bq")){$(".js-JVC").viewportChecker({classToAdd:"s-is-visible",offset:96,repeat:false})}}j();n.on("resize",function(){clearTimeout(m);m=setTimeout(j,300)})}}function f(){$("a.tappilyTap").bind("tap",function(j){window.location=j.target.href})}function g(){var k=$(".js-openmodal");var j=$(".js-closemodal");k.bind("tap",function(m){var l=$(this);var n=$(l.attr("data-modaltarget"));n.addClass("s-is-active")});j.bind("tap",function(){var m=$(this);var l=m.parents(".o-modal");l.removeClass("s-is-active")})}function b(){$(".js-jumper").bind("tap",function(k){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var j=$(this.hash);j=j.length&&j||jQuery("[name="+this.hash.slice(1)+"]");if(j.length){var l=j.offset().top-24;$("html,body").animate({scrollTop:l},750)}}})}function d(){if($(".m-toggled").length){var k=$(".m-toggled");k.each(function(){var m=$(this);if(m.is("ul")){var l=m.clone(true);l.find("li").slice(1).wrapAll('<li class="m-toggled__elem s-is-hidden"><ul></ul></li>');l.find("li:eq(0) > *").addClass("a-toggled__toggler");m.replaceWith(l)}});var j=$(".m-toggled__elem").prev();j.bind("tap",function(){var n=$(this);var m=n.parent(".m-toggled");var l=n.next(".m-toggled__elem");m.toggleClass("s-is-active");l.slideToggle("fast").removeClass("s-is-hidden")})}}function e(){if($(".m-expand-section").length){var k=$(".m-expand-section");var j=k.innerHeight();var l=k.attr("data-divide-by");var m=k.attr("data-expand-text");var n=$('<button class="a-expand-section__button">'+m+"</button>");k.css("max-height",j/l);n.insertAfter(k);n.bind("tap",function(){var o=$(this);o.prev(k).css("max-height",j).addClass("s-is-expanded");o.addClass("s-is-hidden")})}}function a(){if($(".m-form").length){function j(){$("input:checked").parent(".a-label").addClass("s-is-checked");$("input:disabled").parent(".a-label").addClass("s-is-disabled");$('form[role="form"]').on("click",".a-label.checkbox, .a-label.radio",function(){var l=$(this);if(l.children().is(":disabled")){return false}var k=l.children("input:checkbox");if(k.length){if(k.is(":checked")){l.addClass("s-is-checked")}else{l.removeClass("s-is-checked")}}else{var m=l.siblings();m.removeClass("s-is-checked");l.addClass("s-is-checked")}})}j()}}function i(){if($(".js-imgLoader").length){var j=$(".js-imgLoader");j.each(function(m){var o=$(this);var l=o.find("img").eq(0);var n=new Image();var k=o.attr("data-loader");if(k=="true"){var p=$('<p class="js-loader"></p>');o.parent().css("position","relative");o.parent().append(p)}$(n).on("load",function(){setTimeout(function(){p.fadeOut("fast");o.removeClass("s-is-hidden")},m*300)});n.src=l.attr("src")})}}function c(){if($(".js-videoParent").length){var j=$(".js-videoParent");j.fitVids()}}h();f();g();b();d();e();a();i();c();setTimeout(function(){var j=[".a-components__title",".example-2"].join(", ");$(j).css({visibility:"visible",opacity:"1",transform:"translate3d(0,0,0)"})},1000)})();