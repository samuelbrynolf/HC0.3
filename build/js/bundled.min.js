...
(function(c,f,g){c.tapHandling=false;var d=function(h){return h.each(function(){var s=f(this),m,p,q,r,i=10;function k(t){f(t.target).trigger("tap",[t,f(t.target).attr("href")]);t.stopImmediatePropagation()}function o(v){var t=v.originalEvent||v,u=t.touches||t.targetTouches;if(u){return[u[0].pageX,u[0].pageY]}else{return null}}function j(u){if(u.touches&&u.touches.length>1||u.targetTouches&&u.targetTouches.length>1){return false}var t=o(u);q=t[0];p=t[1]}function l(u){if(!r){var t=o(u);if(t&&(Math.abs(p-t[1])>i||Math.abs(q-t[0])>i)){r=true}}}function n(t){clearTimeout(m);m=setTimeout(function(){c.tapHandling=false;r=false},1000);if((t.which&&t.which>1)||t.shiftKey||t.altKey||t.metaKey||t.ctrlKey){return}t.preventDefault();if(r||c.tapHandling&&c.tapHandling!==t.type){r=false;return}c.tapHandling=t.type;k(t)}s.bind("touchstart.tappy MSPointerDown.tappy",j).bind("touchmove.tappy MSPointerMove.tappy",l).bind("touchend.tappy MSPointerUp.tappy",n).bind("click.tappy",n)})};var a=function(h){return h.unbind(".tappy")};if(f.event&&f.event.special){f.event.special.tap={add:function(h){d(f(this))},remove:function(h){a(f(this))}}}else{var b=f.fn.bind,e=f.fn.unbind;f.fn.bind=function(h){if(/(^| )tap( |$)/.test(h)){d(this)}return b.apply(this,arguments)};f.fn.unbind=function(h){if(/(^| )tap( |$)/.test(h)){a(this)}return e.apply(this,arguments)}}}(this,jQuery));(function(){$("html").addClass("transitions");function g(){if($("#getActiveMQ-watcher").length){var m=$(window);var l=0;var n=$("#js-mq-example");var k=n.text();function j(){n.text("Yey! Alpha mode!")}function i(){var o=getActiveMQ();$("#js-mq-currentexample").text("Current screen: "+o);if(o=="aq"){j()}else{n.text(k)}if(!(o=="aq")&&!(o=="bq")){$(".js-JVC").viewportChecker({classToAdd:"s-is-visible",offset:96,repeat:false})}}i();m.on("resize",function(){clearTimeout(l);l=setTimeout(i,300)})}}function d(){$("a.tappilyTap").bind("tap",function(i){window.location=i.target.href})}function a(){$(".js-jumper").bind("tap",function(j){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var i=$(this.hash);i=i.length&&i||jQuery("[name="+this.hash.slice(1)+"]");if(i.length){var k=i.offset().top-24;$("html,body").animate({scrollTop:k},750)}}})}function f(){if($(".m-toggled").length){var j=$(".m-toggled");j.each(function(){var l=$(this);if(l.is("ul")){var k=l.clone(true);k.find("li").slice(1).wrapAll('<li class="m-toggled__elem s-is-hidden"><ul></ul></li>');k.find("li:eq(0) > *").addClass("a-toggled__toggler");l.replaceWith(k)}});var i=$(".m-toggled__elem").prev();i.bind("tap",function(){$this=$(this);var l=$this.parent(".m-toggled");var k=$this.next(".m-toggled__elem");l.toggleClass("s-is-active");k.slideToggle("fast").removeClass("s-is-hidden")})}}function b(){if($(".m-expand-section").length){var j=$(".m-expand-section");var i=j.innerHeight();var k=j.attr("data-divide-by");var l=j.attr("data-expand-text");var m=$('<button class="a-expand-section__button">'+l+"</button>");j.css("max-height",i/k);m.insertAfter(j);m.bind("tap",function(){$this=$(this);$this.prev(j).css("max-height",i).addClass("s-is-expanded");$this.addClass("s-is-hidden")})}}function e(){if($(".m-form").length){function i(){$("input:checked").parent(".a-label").addClass("s-is-checked");$("input:disabled").parent(".a-label").addClass("s-is-disabled");$('form[role="form"]').on("click",".a-label.checkbox, .a-label.radio",function(){$this=$(this);if($this.children().is(":disabled")){return false}var j=$this.children("input:checkbox");if(j.length){if(j.is(":checked")){$this.addClass("s-is-checked")}else{$this.removeClass("s-is-checked")}}else{var k=$this.siblings();k.removeClass("s-is-checked");$this.addClass("s-is-checked")}})}i()}}function h(){if($(".js-imgLoader").length){var i=$(".js-imgLoader");i.each(function(l){var n=$(this);var k=n.find("img").eq(0);var m=new Image();var j=n.attr("data-loader");if(j=="true"){var o=$('<p class="js-loader"></p>');n.parent().css("position","relative");n.parent().append(o)}$(m).on("load",function(){setTimeout(function(){o.fadeOut("fast");n.removeClass("s-is-hidden")},l*300)});m.src=k.attr("src")})}}function c(){if($(".js-videoParent").length){var i=$(".js-videoParent");i.fitVids()}}g();d();a();f();b();e();h();c();setTimeout(function(){var i=[".a-components__title",".example-2"].join(", ");$(i).css({visibility:"visible",opacity:"1",transform:"translate3d(0,0,0)"})},1000)})();